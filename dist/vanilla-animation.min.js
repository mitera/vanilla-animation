...
/**
	     ...
	     */initScrollEvent(){let $this=this;window.addEventListener("scroll",(function(){$this.offsetItems.forEach((function(item){let offset=item.dataset.vanimationOffset?parseInt(item.dataset.vanimationOffset):$this.settings.offset;if(item.dataset.vanimationScrolly){if(parseInt(item.dataset.vanimationScrolly)+offset<window.scrollY){item.removeAttribute("data-vanimation-scrolly");$this.doAnimation(item);const index=$this.offsetItems.indexOf(item);if(index>-1){$this.offsetItems.splice(index,1)}}}}))}))}
/**
	     ...
	     */resetItems(){let $this=this;this.items.forEach((function(item){$this.resetItem(item)}))}
/**
	     ...
	     * @return {void} This method does not return a value.
	     */resetItem(item){item.style.visibility="hidden";let animationName=window.getComputedStyle(item).getPropertyValue("animation-name");if(animationName&&animationName!="none"){item.classList.remove(this.settings.animatePrefix+animationName);item.dataset.vanimation=this.settings.animatePrefix+animationName}}
/**
	     ...
	     */intertsectionObserve(){let $this=this;let itemObserver=new IntersectionObserver((function(entries){entries.forEach((function(entry){if(entry.isIntersecting){let item=entry.target;if(!item.classList.contains($this.settings.animateClass)){if(!$this.isVisible(item)){item.dataset.vanimationScrolly=window.scrollY+"";$this.offsetItems.push(item)}else{$this.doAnimation(item)}}}}))}));return itemObserver}
/**
...
/**
	     ...
	     */parseDuration(duration){if(!duration)return 0;if(duration.indexOf("ms")>-1){return parseInt(duration.replace("ms",""))}let intDuration=parseInt(duration.replace("s",""));return intDuration}
/**
...
/**
	     ...
	     */isMobile(agent){return/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(agent)}
/**
